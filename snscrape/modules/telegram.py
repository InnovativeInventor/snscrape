    ...
    def _soup_to_items(self, soup, pageUrl):
        ...
        for post in reversed(posts):
            date = datetime.datetime.strptime(
                post.find("div", class_="tgme_widget_message_footer")
                .find("a", class_="tgme_widget_message_date")
                .find("time", datetime=True)["datetime"]
                .replace("-", "", 2)
                .replace(":", ""),
                "%Y%m%dT%H%M%S%z",
            )
            ...
            if message:
                ...
                outlinks = [
                    urllib.parse.urljoin(pageUrl, link["href"])
                    for link in post.find_all("a")
                    if not link.text.startswith("@")
                    and link["href"].startswith("https://t.me/")
                ]
                ...
                outlinksss = ""
            yield TelegramPost(
                url=f'https://t.me/s/{post["data-post"]}',
                date=date,
                content=content,
                outlinks=outlinks,
                outlinksss=outlinksss,
            )
