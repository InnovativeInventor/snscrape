                ...
                for entry in entries:
                    if entry["entryId"].startswith("sq-I-t-"):
                        if "tweet" in entry["content"]["item"]["content"]:
                            ...
                            and "tweet"
                            in entry["content"]["item"]["content"]["tombstone"]
                        ):
                            tweet = obj["globalObjects"]["tweets"][
...
class TwitterThreadScraper(TwitterCommonScraper):
    ...
    def get_items(self):
        headers = {
            "User-Agent": f"Opera/9.80 (Windows NT 6.1; WOW64) Presto/2.12.388 Version/12.18"
        }
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    def setup_parser(cls, subparser):
        subparser.add_argument(
            "list", help='A Twitter list, formatted as "username/listname"'
        )
...
class TwitterListMembersScraper(TwitterCommonScraper):
    ...
    def get_items(self):
        ...
        if (
            not container.has_attr("data-min-position")
            or container["data-min-position"] == ""
        ):
            return
