...
def parse_args():
    ...
    parser.add_argument(
        "-f", "--format", dest="format", type=str, default=None, help="Output format"
    )
    parser.add_argument(
        "--since",
    ...
    subparsers = parser.add_subparsers(
        dest="scraper", help="The scraper you want to use"
    )
    classes = snscrape.base.Scraper.__subclasses__()
    for cls in classes:
        if cls.name is not None:
            subparser = subparsers.add_parser(
                cls.name, formatter_class=argparse.ArgumentDefaultsHelpFormatter
            )
